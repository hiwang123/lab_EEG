function [R_old,mu_old,sigma_old,pi_old,log_lik_list,count]=EM_MoG(Data,mu_init,sigma_init,pi_init,dim,N_clusters,N_points,N_iteration)
...
%
% This is a demonstratable verison. Copyright Yen-Chun Chou 04/17/2006
...
while count < N_iteration
    ...
    if sum(total_pr == 0) ~= 0
       fprintf('total_pr has zero elements. Replace them by random numbers !\n');
       ind0 = find(total_pr == 0);
       total_pr(ind0) = 0.001*rand(size(ind0));
    end
    ...
    if (count>1) && (log_lik_list(count)-log_lik_list(count-1)<1e-6)
        fprintf('Iteration = %g\nBye!\n',count);
        break;
    end
end